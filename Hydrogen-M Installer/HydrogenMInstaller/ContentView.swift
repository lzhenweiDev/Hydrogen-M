...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
                ...
                if !installer.logMessages.isEmpty {
                    VStack(spacing: 0) {
                        Button(action: { withAnimation(.easeInOut(duration: 0.2)) { showLog.toggle() } }) {
                            HStack {
                                Label(showLog ? "Hide Details" : "Show Details", systemImage: showLog ? "chevron.up" : "chevron.down")
                                    .font(.caption)
                                Spacer()
                            }
                            .foregroundColor(.secondary)
                        ...
                        if showLog {
                            ScrollView {
                                VStack(alignment: .leading, spacing: 2) {
                                    ForEach(installer.logMessages, id: \.self) { msg in
                                        Text(msg)
                    ...
                    .installerGlassCard(cornerRadius: 14, fillOpacity: 0.5)
                    .padding(.horizontal, 24)
                    .padding(.top, 12)
                }
            ...
            .frame(minWidth: 460, idealWidth: 500, maxWidth: 560, minHeight: 620, idealHeight: 680)
            .background(
